...
> {
    ...
    setRows: (rows: Array<RowDataType>) => void;
    updateVisibleRows: (callback: (visibleRows: Array<RowDataType>) => Array<RowDataType>) => void;

    visibleRows: ReadonlyArray<RowDataType>;
    setVisibleRows: (rows: Array<RowDataType>) => void;
...
export type DataGridDefaultLayoutProps<RowDataType extends object = AnyObject> = {
    // Панель фильтрации.
    ...
    // Настройка таблицы.
    tableProps?: Partial<Omit<DataGridTableProps<RowDataType>, 'renderHeaders' | 'renderRow' | 'flexFill'>>;
...
export interface DataGridFiltersPanelButtonsProps extends Omit<HTMLAttributes<HTMLDivElement>, 'children'> {
    disabled?: boolean;
...
export interface DataGridFiltersPanelLabelProps extends Omit<HTMLAttributes<HTMLDivElement>, 'children'> {
    label?: string;
}

// Свойства панели сортировки данных в таблице.
export interface DataGridOrderingPanelProps<SortableColumn extends string = string> extends Omit<HTMLAttributes<HTMLDivElement>, 'children'> {
...
export interface DataGridOrderingPanelOption<ValueType extends string = string> extends FormSelectOption<ValueType> {
    type?: DataGridOrderingPanelOptionType;
}

// Свойства заголовка панели сортировки.
export interface DataGridOrderingPanelLabelProps extends Omit<HTMLAttributes<HTMLDivElement>, 'children'> {
    label?: string;
}
...
export interface DataGridFooterProps extends Omit<DataGridFooterWrapperProps, 'children'> {
    disabled?: boolean;
    limitChanger?: boolean;
    paginationProps?: Omit<DataGridPaginationProps, 'offset' | 'limit' | 'disabled' | 'onOffsetChange' | 'totalCount'>;
}
...
export interface DataGridHeaderProps<OrderByOptions extends string = string> extends Omit<DataGridHeaderWrapperProps<OrderByOptions>, 'onClick'> {
    sortingDataType?: DataGridOrderByValuesType;
    resetOffset?: boolean;
}

// Свойства заголовков колонок таблицы (<thead> -> <tr> -> <DataGridHeader>+).
export type DataGridHeadersProps = {
    children: React.ReactNode | React.ReactNode[];
    hidden?: boolean;
    sticky?: boolean;
}
...
export interface DataGridItemsCountProps extends Omit<HTMLAttributes<HTMLDivElement>, 'children'> {
    totalCount: number;
...
export interface DataGridPaginationProps extends Omit<HTMLAttributes<HTMLDivElement>, 'children'> {
    totalCount: number;
    offset: number;
    limit: number;
    disabled?: boolean;
